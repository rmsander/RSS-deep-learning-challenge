...
def parse_args():
    ...
    parser.add_argument('-i', '--images_dir', default=r'C:\Users\mitadm\Documents\Ryan\6.141\final_challenge\keras-yolo3\ML_pipeline\blue_line', help='Image directory')
...
def check_blue_line(frame,img_name): #Check to see if a blue line(s) exists in a frame
    ...
    if True: #Try to detect blue line
        ...
        for contour in contours: #There may be multiple lines; we should always take the one closest to us
            ...
            if v > y_max/3:
                ...
                (x2,y2) = (u+w,v+h/2)
                #print("Area is",w*h)
                xl,yl = homography.apply_homography(x1,y1) #Left end point of blue line in world space
                xr,yr = homography.apply_homography(x2,y2) #Right end point of blue line in world space
        ...
        try:
            ...
            if A > 5000 and (x2-x1)/(y2-y1) > 5: #Detection
                return points[contour_index],(xl+xr)/2 #Returns xl,yl,xr,yr
            else:
                return None,None
        except:
            return None,None
...
def main():
    ...
    for img_name in filtered_files[args.begin_index:args.begin_index+args.num_images]:
        ...
        line,dist = check_blue_line(frame,img_name)
