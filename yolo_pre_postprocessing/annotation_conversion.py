...
raw_data_path = "ML_pipeline/raw_data_copy5/"
...
#             try:
#                 os.remove(cwd+"\\ML_pipeline\\raw_training_data)copy
#                 \\"+line[:-1])
#             except:
...
for i in range(len(image_names)):
    print(image_names[i])
    train.write(cwd + "\\cus\\images\\" + "ds_" + image_names[i])
    train.write("\n")
train.close()
...
classes2numbers = {"TRAFFIC_LIGHT": 0, "PERSON": 1, "STOP_SIGN": 2,
                   "YIELD_SIGN": 3, "CONE": 4, "PEDESTRIAN_SIGN": 5,
                   "DISABLED_PARKING_SIGN": 6, "DISABLED_PARKING_SPACE": 6}
classes2numbers2 = {"DISABLED_PARKING_SIGN": 0, "CONE": 1, "YIELD_SIGN": 2,
...
def format_1():
    for i in range(len(assets)):
        ...
        for data in dic:
            ...
            y_cen = t + h / 2
            file.write(
                str(label) + " " + str(x_cen) + " " + str(y_cen) + " " + str(
                    w) + " " + str(h))
        file.close()
...
for i in range(len(assets)):
    ...
    for data in dic:
        ...
        x_max = l + w
        y_max = t + h
        file.write(str(int(x_min)) + "," + str(int(y_min)) + "," + str(
            int(x_max)) + "," + str(int(y_max)) + "," + str(label) + " ")
    file.write("\n")
file.close()
